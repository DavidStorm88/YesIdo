...
class GimnasioSupabaseUser extends BaseAuthUser {
  GimnasioSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> gimnasioSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = GimnasioSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
